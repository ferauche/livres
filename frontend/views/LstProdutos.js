Vue.component('LstProdutos', {
    ...
    created() {
        service.getProdutores()
            .then(r => this.produtores = r)
    },
    methods: {
        changeProdutor() {
            service.getProdutosByProdutorId(this.produtor)
        ...
        updateCarrinho(produto) {
            service.updateCarrinho({ id: produto.id, qtd: produto.qtd })
                .then(r => {})
            ...
            <div class="col-8 offset-md-2">
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="pt-2"></div>
                        <div class="select">
                            <select class="select-text" required  v-model.trim="produtor" @change="changeProdutor()">
                                <option disabled selected></option>
                                <option v-for="option in produtores" v-bind:value="option.id">
                                    {{ option.value }}
                                </option>
                            </select>
                        ...
                        <div class="table-responsive">
                            <table id="dtBasicExample" class="table table-striped table-bordered table-sm" cellspacing="0" width="100%" v-show="produtos.length">
                        ...
                        <div class="row">
                            <div class="col">
                                <button class="btn btn-primary pull-right" @click="$router.push('/checkout')"><i class="fa fa-shopping-basket" aria-hidden="true"></i> Ir até o Carrinho</button>
                            </div>
                        </div>
                        <pre>Data: {{ $data }}</pre>
                    </div>
